...
import { SEARCH_API, YOUTUBE_SEARCH_VIDEO_WITH_QUERY_API } from "../utils/constant";
...
function Nav() {
  ...
  async function fetchSearchRecom(){
    const res = (await axios.get(YOUTUBE_SEARCH_VIDEO_WITH_QUERY_API + query))
    const data = res.data
    console.log(data)  
  }
  ...
  return (
    <nav className="flex w-full justify-around items-center border-b-2 p-2">
      ...
      <div className="w-1/3 flex">
        ...
        <button onClick={ fetchSearchRecom } className="border p-2 px-4 rounded-e-full">
          <img src={search} alt="" />
        </button>
      </div>
